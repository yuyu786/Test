...
async def rcloneNode(client, message, link, dst_path, rcf, tag):
    ...
    if link.startswith('mrcc:'):
        ...
        config_path = f'bridi/{message.from_user.id}.conf'
    else:
    ...
    if dst_path.startswith('mrcc:'):
        if config_path != f'bridi/{message.from_user.id}.conf':
            await sendMessage(message, 'You should use same rcl.conf to clone between paths!')
            return
...
async def clone(client, message):
    ...
    if is_rclone_path(link):
        if not await aiopath.exists('rcl.conf') and not await aiopath.exists(f'bridi/{message.from_user.id}.conf'):
